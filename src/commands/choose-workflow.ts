  ...
  try {
    ...
    const tasks = config.workflow.map((task, index) => {
      ...
      if (taskType === "prompt") {
        ...
        const cmds = Array.isArray(task.commands)
          ? task.commands.join(", ")
          : task.command || "";
        description += `: ${cmds}`;
      } else if (taskType === "npm_install") {
        const pkgs = Array.isArray(task.packages)
          ? task.packages.join(", ")
          : "";
        description += `: ${pkgs}`;
        ...
        taskType === "ai_modify_file" ||
        taskType === "ai_merge_file"
      ) {
    ...
    logger.info(
      `Running ${selectedIndices.length} selected tasks from ${file}`
    );
