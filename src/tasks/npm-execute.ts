...
export class NpmExecuteTask implements Task {
  public async execute(taskContext: TaskContext): Promise<void> {
    ...
    if (cmds.length === 0) {
      throw new Error(
        'NpmExecute task requires either "command" or "commands" property'
      );
    }
    ...
    if (pm === "auto") {
      try {
        ...
        logger.warn(
          `Failed to detect package manager, using npm: ${(e as Error).message}`
        );
        pm = "npm";
      }
    }
  ...
  public validate(config: CommonTaskConfig): boolean {
    return Boolean(
      config.command || (config.commands && Array.isArray(config.commands))
    );
  }
