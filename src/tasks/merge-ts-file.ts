...
export class MergeTsFileTask implements Task {
  ...
  private smartMerge(target: string, source: string): string {
    ...
    const sourceSourceFile = ts.createSourceFile(
      'source.ts',
      source,
      ts.ScriptTarget.Latest,
      true
  ...
  private combineContent(
    ...
    for (const importStmt of this.extractImports(ts.createSourceFile('target.ts', targetContent, ts.ScriptTarget.Latest, true))) {
      cleanedTargetContent = cleanedTargetContent.replace(importStmt, '');
    }
    
    for (const exportStmt of this.extractExports(ts.createSourceFile('target.ts', targetContent, ts.ScriptTarget.Latest, true))) {
    ...
    for (const importStmt of this.extractImports(ts.createSourceFile('source.ts', sourceContent, ts.ScriptTarget.Latest, true))) {
      cleanedSourceContent = cleanedSourceContent.replace(importStmt, '');
    }
    
    for (const exportStmt of this.extractExports(ts.createSourceFile('source.ts', sourceContent, ts.ScriptTarget.Latest, true))) {
    ...
    return [
      ...imports,
      '',
      cleanedTargetContent.trim(),
      '',
      cleanedSourceContent.trim(),
      '',
      ...exports
    ].join('\n');
  }
