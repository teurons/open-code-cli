...
import {
  ...
  ContributeSyncResult,
  ContributeOperationType,
} from "./types";
...
export async function executeContributeSyncOperations(
    ...
    try {
      if (dryRun) {
        ...
        if (operationType === "copy") {
          ...
          logger.info(
            `[${repo}] Attempting to delete file: ${absoluteSourcePath}`
          );

          if (existsSync(absoluteSourcePath)) {
            logger.info(
              `[${repo}] File exists, deleting: ${absoluteSourcePath}`
            );
            ...
            logger.info(
              `[${repo}] File doesn't exist, skipping: ${absoluteSourcePath}`
            );
            results.push({
              operation,
              success: true,
  ...
  const successCopy = results.filter(
    r => r.success && r.action === "copy"
  ).length;
  const successDelete = results.filter(
    r => r.success && r.action === "delete"
  ).length;
