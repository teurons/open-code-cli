...
export async function handleContributeCommand(
  ...
  try {
    ...
    if (!isInstalled) {
      ...
      logger.info(
        "You can install it from: https://cli.github.com/manual/installation"
      );
      process.exit(1);
    }
    ...
    if (!existsSync(trackerPath)) {
      logger.error(
        red(
          "Tracker file not found. Please run gh-sync first to create a tracker file."
        )
      );
      process.exit(1);
    }
    ...
    const prBody =
      "This PR contains changes contributed from local to source, including:";
    ...
    for (const { repo, forkRepo, filePaths } of reposWithFork) {
      ...
      if (isSelfFork) {
        logger.info(
          `Fork repository is the same as source repository (${repo})`
        );
      }
      ...
      if (repoData?.pullRequest) {
        ...
        if (currentPRStatus && currentPRStatus.status === "open") {
          ...
          logger.info(
            `PR #${prInfo.prNumber} is closed or merged, will create a new PR`
          );
        }
      }
      ...
      try {
        ...
        if (updateExistingPR && forkRepo !== repo) {
          ...
          logger.info(
            `Fork repository is the same as source repository (${repo}), skipping sync`
          );
        }
        ...
        if (updateExistingPR) {
          ...
          if (branchSuccess) {
            ...
            if (!mergeSuccess) {
              logger.warn(
                `Failed to merge main branch into ${branchName}, continuing anyway`
              );
            } else {
