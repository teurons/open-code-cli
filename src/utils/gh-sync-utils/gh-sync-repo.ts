...
export async function processRepo(
  ...
  const shouldFetchResult = await checkIfShouldFetch(
    repo,
    branch,
    sync,
    force,
  ...
  try {
    ...
    const syncOperations = collectSyncOperations(
      repoGroup.files,
      tempDir,
      cwd,
      repo
    ...
    for (const file of repoGroup.files) {
      ...
      if (existsSync(sourcePath) && statSync(sourcePath).isDirectory()) {
        const removedFiles = await handleDeletedFiles(
          sourcePath,
          localPath,
          cwd,
          repo
    ...
    logger.error(
      `Error processing repository ${repo}: ${(error as Error).message}`
    );
    hasError = true;
    return {
      repo,
...
async function checkIfShouldFetch(
  ...
  try {
    ...
    if (sync && !force) {
      ...
      const lastSyncedCommit =
        repoData?.branch === branch ? repoData.lastCommitHash : null;
    ...
    logger.warn(
      `Failed to check for updates in repository ${repo}: ${(e as Error).message}`
    );
...
function updateTrackerWithLatestData(
  ...
  if (
    fileData &&
    fileData[repo] &&
    Object.keys(fileData[repo] || {}).length > 0
  ) {
    ...
    if (trackerConfig.repos[repo]?.files) {
      for (const filePath in trackerConfig.repos[repo].files) {
        const fileData = trackerConfig.repos[repo].files[
          filePath
