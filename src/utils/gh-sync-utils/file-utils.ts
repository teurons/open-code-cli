...
export function getFileHash(filePath: string): string {
  ...
  try {
    ...
    logger.warn(
      `Failed to calculate hash for ${filePath}: ${(e as Error).message}`
    );
    return "";
  }
}
...
export function actionOnFile(
  ...
  const trackerFileHash =
    trackerConfig.repos[repo]?.files?.[relativeLocalPath]?.hash || null;
  const trackerAction =
    trackerConfig.repos[repo]?.files?.[relativeLocalPath]?.action || null;
...
export async function handleDeletedFiles(
  ...
  if (repo && cwd) {
    ...
    if (trackerConfig.repos[repo] && trackerConfig.repos[repo].files) {
      trackedFiles = new Set<string>(
        Object.keys(trackerConfig.repos[repo].files)
      );
    }
  }
  ...
  logger.info(
    `Found ${filesToDelete.length} files that exist locally but were deleted in source.`
  );

  const deleteOption = await logger.prompt(
    "How would you like to handle deleted files?",
    {
      type: "select",
  ...
  for (const file of filesToRemove) {
    try {
      ...
      logger.info(
        `Removed file that was deleted in source: ${file.relativePath}`
      );
      removedFiles.push(file.relativePath);
    } catch (e) {
      logger.warn(
        `Failed to remove file ${file.path}: ${(e as Error).message}`
      );
    }
  }
  ...
  const cleanEmptyDirs = (dir: string) => {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (
          existsSync(fullPath) &&
          readdirSync(fullPath, { withFileTypes: true }).some(e =>
            e.isDirectory()
          )
        ) {
  ...
  try {
    ...
    readdirSync(sourceDir, { withFileTypes: true }).forEach(entry => {
      ...
      if (entry.isDirectory()) {
        ...
        const relativeSourcePath = tempDir
          ? relative(tempDir, sourcePath)
          : sourcePath;
    ...
    throw new Error(
      `Failed to sync directory ${sourceDir}: ${(error as Error).message}`
    );
  }
}
